...
import { getRandomPromise } from '@/lib/promises';
...
export default function Home() {
  ...
  const refreshVerse = useCallback(async () => {
    try {
      const randomVerse = getRandomPromise();
      setVerse(randomVerse);

      if (randomVerse.struct) {
        setLoading(true);
        const result = await fetchVerseContent(
          globalSettings.bibleVersion,
          randomVerse.struct.book,
          randomVerse.struct.chapter,
          randomVerse.struct.verses
        );
        if (result) {
          setDynamicText(result.text);
        } else {
          setDynamicText(randomVerse.text);
        }
        setLoading(false);
      } else {
        setDynamicText(randomVerse.text);
      ...
      console.error("Error picking random verse", e);
    }
  }, [globalSettings.bibleVersion]);
          ...
          <View className="p-8">
            <View className="flex-row items-center justify-between mb-6">
              <View className="bg-white/20 px-3 py-1.5 rounded-full">
                <Text className="text-[10px] font-bold uppercase text-white tracking-widest">Teny Fikasana</Text>
              </View>
                  ...
                  onPress={() => {
                    if (verse) {
                      router.push({
                        ...
                        params: {
                          verseText: dynamicText || verse.text,
